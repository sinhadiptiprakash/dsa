...
using namespace std;
//using extra space
void merge_two_sorted_arrays(int a[], int n, int b[], int m)
{
...
//below is the insertion sort approach without using extra space
...
int nextGap(int gap)
{
    if (gap <= 1)
        return 0;
    // adding remainder to the gap as we are dealing with ceil(gap)
    return (gap / 2) + (gap % 2);
}
...
int main()
{
    ...
    merge_two_sorted_arrays_without_extra_space(a, b, n, m);
    for (int i = 0; i < n; i++)
        cout << a[i] << ' ';
    cout << endl;
    for (int i = 0; i < m; i++)
