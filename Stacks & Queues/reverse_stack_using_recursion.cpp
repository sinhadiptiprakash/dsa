...
void insert_at_bottom(char x)
{
    ...
    {
        // All items are held in Function Call Stack until we reach end of the stack When the stack becomes empty, the
        // st.size() becomes 0, the above if part is executed and the item is inserted at the bottom
...
// Below is the function th reverses the given stack usi insert_at_bottom()
void reverse()
{
    if (st.size() > 0)
    {
        ...
        // Insert all the items in Function Call Stack one by one from the bottom to top. Every item is inserted at the bottom
        insert_at_bottom(x);
    }
}
